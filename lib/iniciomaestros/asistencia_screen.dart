...
class _AsistenciaScreenState extends State<AsistenciaScreen> with TickerProviderStateMixin {
  String currentTime = "";
  String currentDate = "";
  ...
  List<Color> materiaColors =
      List.filled(6, Colors.blue); // Inicializar con azul
  ...
  List<String> materias = []; // Lista de materias creadas en MateriasScreen
  ...
  void initState() {
    ...
    updateDateTime();
    calculateProgress();
    cargarMaterias(); // Cargar las materias al iniciar la pantalla
  ...
  Future<void> cargarMaterias() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String>? materiasJson = prefs.getStringList('materias');
    if (materiasJson != null) {
      setState(() {
        materias = materiasJson.toList();
      });
    }
  }
